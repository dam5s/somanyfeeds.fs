...
type FieldError<'Error> = { FieldName: string; Error: 'Error }

type Validation<'Success, 'Failure> = Result<'Success, FieldError<'Failure> list>

[<RequireQualifiedAccess>]
module Validation =

    let error name err = [ { FieldName = name; Error = err } ]
